-- BSB Digital Menu — Phase 1 table-wise order foundation (FIXED).
...
--
-- Fix vs table-wise-order.sql:
-- Policy "waiters read own assignments" compared waiters.id (uuid) to the inner
-- waiters.waiter_id (text) because unqualified waiter_id was resolved on waiters.
-- All waiter FK comparisons now use table-qualified uuid columns.
...
create policy "waiters read own assignments"
  ...
  using (
    exists (
      ...
      where w.id = waiter_table_assignments.waiter_id
...
create policy "waiters insert orders"
  ...
  with check (
    ...
    and (orders.waiter_id is null or orders.waiter_id = public.current_waiter_id())
